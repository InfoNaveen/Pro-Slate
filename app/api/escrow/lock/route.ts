...
import { generateMockTxHash } from '@/lib/escrow-mock';
import { z } from 'zod';

const schema = z.object({ job_id: z.string().uuid(), amount: z.number().positive() });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { job_id, amount } = schema.parse(body);
